...
DROP POLICY IF EXISTS "Doctors at target hospital can view referrals" ON public.referrals;
DROP POLICY IF EXISTS "Doctors can update referrals they created" ON public.referrals;
DROP POLICY IF EXISTS "Doctors at target hospital can update referrals" ON public.referrals;

-- Allow doctors to view referrals targeted to their hospital
CREATE POLICY "Doctors at target hospital can view referrals"
ON public.referrals
FOR SELECT
TO authenticated
USING (
...
-- Allow doctors to update referrals they created
CREATE POLICY "Doctors can update referrals they created"
ON public.referrals
FOR UPDATE
TO authenticated
...
-- Allow doctors at target hospital to update referrals (e.g., mark as completed)
CREATE POLICY "Doctors at target hospital can update referrals"
ON public.referrals
FOR UPDATE
TO authenticated
USING (
